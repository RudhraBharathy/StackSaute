      ...
      className={`
        ...
        ${isDisabled
          ...
          : 'cursor-pointer ' + (isSelected
            ...
            : 'border-gray-800 bg-gray-900 hover:border-gray-600 hover:bg-gray-800 transition-colors duration-300')
        }
      `}
    >
      <div className="flex justify-between items-start">
        <div className={`text-3xl ${isSelected ? 'text-orange-500' : 'text-gray-400 group-hover:text-white transition-colors duration-300'}`}>
          {ingredient.icon}
        </div>
        {isSelected && (
          <div className="w-5 h-5 bg-orange-500 rounded-full flex items-center justify-center transition-colors duration-300">
      ...
      <div>
        <h3 className={`text-lg font-bold mb-1 ${isSelected ? 'text-white' : 'text-gray-300 transition-colors duration-300'}`}>
          {ingredient.name}
        </h3>
